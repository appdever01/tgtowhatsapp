...
const apiKey = process.env.OPENAI_KEY || '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'
        ...
        const promises = channels.map(async (channel, channelIndex) => {
            ...
            if (index !== -1) {
                ...
                messagesToSend.forEach(async (message, messageIndex) => {
                    ...
                    const keywords = await getKeyWords(text.includes(errorMessage) ? caption : text)
                    saveWords(keywords)
                    text = `*${channel}*\n\n${text}`
                    const replyData = type === 'text' ? text : { url: mediaUrl }
                    if (skipper && text.includes(errorMessage)) {
                        client.log(`Skipping translation failed message from ${channel}`, true)
                        return void null
                    }
